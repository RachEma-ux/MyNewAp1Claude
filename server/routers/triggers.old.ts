...
import { triggerRegistry, actionRegistry } from "../../drizzle/schema";
import { eq, and, desc, sql } from "drizzle-orm";
import { TRPCError } from "@trpc/server";

// Validation schema for trigger creation
const triggerCreateSchema = z.object({
  // Gate 0: Classification & Intent
  classification: z.enum(["external", "time-based", "manual"]),
  ...
  // Runtime handler
  handlerType: z.enum(["inline", "external", "webhook"]),
...
export const triggersRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (ctx.user.role !== "admin") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Only admins can create trigger types",
        });
      }
      ...
      const existing = await db
        ...
        .where(eq(triggerRegistry.typeId, input.typeId))
        .limit(1);

      if (existing.length > 0) {
        throw new TRPCError({
          code: "CONFLICT",
          message: "Trigger type with this ID already exists",
        });
      }
      ...
      await db.insert(triggerRegistry).values({
        ...input,
  ...
  // List all triggers (with filtering)
  ...
  approve: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== "admin") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Only admins can approve triggers",
        });
      }
  ...
  reject: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== "admin") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Only admins can reject triggers",
        });
      }
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== "admin") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Only admins can delete triggers",
        });
      }
      ...
      await db.delete(triggerRegistry).where(eq(triggerRegistry.id, input.id));
