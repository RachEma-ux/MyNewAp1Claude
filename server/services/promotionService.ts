...
async function signSpec(
  ...
  const secret = process.env.JWT_SECRET || (() => { if (process.env.NODE_ENV === "production" && process.env.DEV_MODE !== "true") throw new Error("JWT_SECRET is required in production"); return "dev-only-secret"; })();
