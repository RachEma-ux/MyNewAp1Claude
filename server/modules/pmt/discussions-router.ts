...
export const discussionsRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const db = getDb();
      if (!db) return [];
      const conditions = [eq(pmDiscussions.workspaceId, input.workspaceId)];
  ...
  get: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const rows = await db.select().from(pmDiscussions)
        .where(and(eq(pmDiscussions.id, input.id), eq(pmDiscussions.workspaceId, input.workspaceId)))
        .limit(1);
  ...
  create: governedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity({ workspaceId: input.workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "discussion.create", targetType: "discussion", targetId: created.id });
      return created;
    }),
  ...
  update: governedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.update(pmDiscussions).set(updates)
        .where(and(eq(pmDiscussions.id, id), eq(pmDiscussions.workspaceId, workspaceId)));
      await logActivity({ workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "discussion.update", targetType: "discussion", targetId: id });
      return { success: true };
    }),
  ...
  delete: governedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.delete(pmDiscussions)
        .where(and(eq(pmDiscussions.id, input.id), eq(pmDiscussions.workspaceId, input.workspaceId)));
      await logActivity({ workspaceId: input.workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "discussion.delete", targetType: "discussion", targetId: input.id });
      return { success: true };
    }),

  posts: router({
    list: protectedProcedure
      .input(z.object({ discussionId: z.number(), workspaceId: z.number().optional() }))
      .query(async ({ input }) => {
        if (input.workspaceId) await requireModule(input.workspaceId, "pmt");
        const db = getDb();
        if (!db) return [];
    ...
    create: governedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logActivity({ workspaceId: input.workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "discussionPost.create", targetType: "discussionPost", targetId: created.id });
        return created;
      }),
    ...
    update: governedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logActivity({ workspaceId: input.workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "discussionPost.update", targetType: "discussionPost", targetId: input.id });
        return { success: true };
      }),
    ...
    delete: governedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logActivity({ workspaceId: input.workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "discussionPost.delete", targetType: "discussionPost", targetId: input.id });
        return { success: true };
      }),
  }),
});
