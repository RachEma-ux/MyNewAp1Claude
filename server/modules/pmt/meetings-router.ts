...
export const meetingsRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const db = getDb();
      if (!db) return [];
      const conditions = [eq(pmMeetings.workspaceId, input.workspaceId)];
  ...
  get: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const rows = await db.select().from(pmMeetings)
        .where(and(eq(pmMeetings.id, input.id), eq(pmMeetings.workspaceId, input.workspaceId)))
        .limit(1);
  ...
  create: governedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity({ workspaceId: input.workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "meeting.create", targetType: "meeting", targetId: created.id });
      return created;
    }),
  ...
  update: governedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const db = getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB unavailable" });
      const { id, workspaceId, startTime, endTime, ...updates } = input;
      ...
      await db.update(pmMeetings).set(setValues)
        .where(and(eq(pmMeetings.id, id), eq(pmMeetings.workspaceId, workspaceId)));
      await logActivity({ workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "meeting.update", targetType: "meeting", targetId: id });
      return { success: true };
    }),
  ...
  delete: governedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.delete(pmMeetings)
        .where(and(eq(pmMeetings.id, input.id), eq(pmMeetings.workspaceId, input.workspaceId)));
      await logActivity({ workspaceId: input.workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "meeting.delete", targetType: "meeting", targetId: input.id });
      return { success: true };
    }),

  items: router({
    list: protectedProcedure
      .input(z.object({ meetingId: z.number(), workspaceId: z.number().optional() }))
      .query(async ({ input }) => {
    ...
    create: governedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logActivity({ workspaceId: input.workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "meetingItem.create", targetType: "meetingItem", targetId: created.id });
        return created;
      }),
    ...
    update: governedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logActivity({ workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "meetingItem.update", targetType: "meetingItem", targetId: id });
        return { success: true };
      }),
    ...
    delete: governedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logActivity({ workspaceId: input.workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "meetingItem.delete", targetType: "meetingItem", targetId: input.id });
        return { success: true };
      }),
  }),
});
