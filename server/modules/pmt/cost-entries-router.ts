...
export const costEntriesRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const db = getDb();
      if (!db) return [];
      const conditions = [eq(pmCostEntries.workspaceId, input.workspaceId)];
  ...
  create: governedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity({ workspaceId: input.workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "costEntry.create", targetType: "cost_entry", targetId: created.id });
      return created;
    }),
  ...
  update: governedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.update(pmCostEntries).set(setValues)
        .where(and(eq(pmCostEntries.id, id), eq(pmCostEntries.workspaceId, workspaceId)));
      await logActivity({ workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "costEntry.update", targetType: "cost_entry", targetId: id });
      return { success: true };
    }),
  ...
  delete: governedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.delete(pmCostEntries)
        .where(and(eq(pmCostEntries.id, input.id), eq(pmCostEntries.workspaceId, input.workspaceId)));
      await logActivity({ workspaceId: input.workspaceId, moduleKey: "pmt", actorId: ctx.user.id, action: "costEntry.delete", targetType: "cost_entry", targetId: input.id });
      return { success: true };
    }),
