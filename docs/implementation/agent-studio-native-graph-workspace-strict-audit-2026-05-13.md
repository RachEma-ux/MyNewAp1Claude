...
| 14 | Phase 11b-3 panel coverage — 17 retention panels extracted | "first slice; 1/17" | **PARTIALLY IMPLEMENTED** (after PR-AT-5) | 8 of 17 panels extracted as standalone components (Runtime/Mcp via #729/#735; Tool/Catalog/Rac via PR-AT-4; Cag/Simulation/Test via PR-AT-5). 9 panels remain inline in `RetrofitPage.tsx`. The remaining 9 still render correctly — extraction is a refactor for testability/maintainability, not a runtime fix. | — | "Retention panel extractions batch 5-N" (incremental) | Panel-coverage + migration-lock source-scan tests green at 8/17 |
...
| PR-AT-5 (Retention panel batch 4) | this PR | Three more panels extracted: `CagPackEventsRetentionPanel` + `SimulationRunsRetentionPanel` + `TestRunsRetentionPanel`. 31 vitest cases. 5/17 → 8/17 |
