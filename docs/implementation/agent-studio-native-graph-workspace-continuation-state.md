...
| #773 PR-V1-22 | OL-3 (V2.0) | OfflineQueue client bootstrap: `getOfflineQueueInstance()` module-singleton + `hydrateOfflineQueueOnLoad()` + `installOfflineDrainListener({ onDrain })` (auto-drain on `window.online` event); `drainNow()` + `uninstall()` handle; SSR/CLI no-op-installer fall-through; 17-test suite | pending |

**Next material work:** WebSocket transport (CRDT-γ-2), MR caller-migration sub-arc, concrete lane hooks (retrieve/assemble/compose), concrete governance-gate adapter (`agsApprovalSteps` wiring), App.tsx wire-up to invoke bootstrap at root. V1.0+V1.5+V2.0 α+β+γ all landed; OfflineQueue is now operational at the client process scope.
