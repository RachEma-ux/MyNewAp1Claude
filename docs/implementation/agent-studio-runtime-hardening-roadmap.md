...
**Phase 11b-2 (CLOSED):** chat.ts (blocking lane) wired with `triggerType="chat"` row + try/catch/finally body wrap that captures thrown errors as `errorReason`. Simulation lane wired with abortReason capture at all 3 abort sites + threaded through the existing terminal `updateRuntimeRun`. Result-typed `{ok:false}` returns from chat.ts intentionally NOT observed at the run level (deferred until a dashboard surfaces a meaningful gap). Test `runtime-observability-writers-chat-sim.test.ts` (14 cases) wired into Layer 8.
