...
| 23 | `graph_correction_rejected` | governance | 🟢 **LIVE @ #1215 (T-I.40)** | `services/graph-correction/lifecycle.ts` reject path — emits on the `decide({decision: "rejected"})` branch (approve / revision_requested / withdrawn do not emit; reject is the only failure-bearing reviewer signal). Audit's prior premise ("No reject path exists") was incorrect — `rejectCorrectionProposal` was shipped in the initial Phase 23 lifecycle work | `services/graph-correction/lifecycle.ts` |
...
- #7 `neo4j_projection_stale` — **DETECTOR + ORCHESTRATOR shipped @ #1218 (T-I.43) + #1219 (T-I.44)** in `services/graph/projection/staleness-detector.ts`. Pure-function detector groups `agsGraphProjectionSyncJobs` rows per projection key, computes max(completedAt) among succeeded runs, partitions into stale / neverSucceeded / freshCount. `runStalenessCheck({ fetchRows, ... })` orchestrator composes fetcher → detector → emitter and is callable from operator tRPC on-demand. The cron-schedule wrapper using `makeRetentionCron` is the remaining slice (defers to a separate PR that touches scheduler boot).
