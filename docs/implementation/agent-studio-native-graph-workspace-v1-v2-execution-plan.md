...
| #770 (PR-V1-19) | Phase 16 wire-up | Vault router gains 3 protected tRPC procedures wiring the 16-β + 16-γ service surfaces: `listVisibleSavedViews` threads `ctx.user.id` as `viewerUserId` into the service; `listSavedViewVersions(savedViewId)` + `getSavedViewVersion(versionId)` expose the immutable history. 11-test source-scan procedure-mount integrity suite at `tests/agent-studio/vault-saved-views-router-procedures.test.ts` | pending |
