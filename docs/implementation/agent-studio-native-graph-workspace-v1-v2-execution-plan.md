...
| #762 (PR-V1-11) | Phase OL-2 | OfflineQueue durability (`client/.../offline-cache-store.ts`): closed `OfflineQueueStore` contract + `InMemoryOfflineQueueStore` (test default + non-browser fall-through) + `IndexedDbOfflineQueueStore` (production; lazy-open db `agent-studio-offline` / store `offline_queue`) + `createOfflineQueueStore()` factory; OfflineQueue gains optional `persistence` adapter + `hydrate()` to restore on reload; no breaking change to OL-1 API | pending |
