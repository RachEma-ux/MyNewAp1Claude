...
| #798 (PR-V1-47) | MR-3 third batch — `ingestion/ingestion-job-service.ts::startJob` | Third migration batch off the MR-3 Phase-1 shim (#794). `startJob(input)` in `server/agent-studio/services/ingestion/ingestion-job-service.ts` migrates to `getAsDbForWorkspace(input.workspaceId)` (StartJobInput.workspaceId is required `number`). The other 2 functions in the same file (`completeJob` / `getJob`) remain on `getAsDb()` — they take only `jobId` and would need workspaceId plumbed through callers (Category B). 4-test source-scan suite at `tests/agent-studio/mr-3-third-batch-ingestion-job-service.test.ts` (import-shape check, `startJob` body assertion, parameterized assertion that the 2 unmigrated functions still call `getAsDb()`). Function-body extractor variant of #794's method extractor — handles top-level `export async function` shape instead of class methods. Out of scope: Phase-2 of the shim, Category B migrations for `completeJob` / `getJob`, the other 3 ingestion files still on `getAsDb()` (separate per-file sub-arcs) | pending |
