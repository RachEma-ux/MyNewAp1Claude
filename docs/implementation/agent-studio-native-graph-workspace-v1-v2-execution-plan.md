...
| #761 (PR-V1-10) | Phase 17-β | Canvas → Note projection edge (`services/canvas/projection.ts`): buildCanvasReferenceProjection emits the 4-write bundle (Canvas + CanvasNode + CONTAINS_CANVAS_NODE + CANVAS_REFERENCES_NOTE); ProjectionSyncWorker handles `canvas.note_reference_changed` / `_removed` event kinds | pending |
