...
|---|---|---|---|---|---|---|---|---|---|---|
| LR-01 | `server/agent-studio/adapters/openllm-runtime-adapter.ts:312-321` | Agent Studio | Reads `process.env[pc.apiKeyEnvVar]` for runtime provider auth. | Phase 17 routed binding-equipped no-tool Expert chat through Model Access; Phase 18 routed binding-equipped TOOL-equipped chat through Model Access via the new `runChatWithToolsViaBinding` (Model Access tool-call schema landed in Phase 18 — `ModelAccessToolCall`). The remaining caller is the simulation engine (`services/simulation.ts`), which is owned by Phase 19 (runtime path migration matrix). | Medium — Expert chat no longer reaches the function for binding-equipped agents (the dominant shape). Simulation engine remains. | `withProviderCredential()` via `openRouter.modelAccess.execute`. | Phase 19 (runtime path migration matrix — simulation engine reclassification) | PR #122 (Phase 18) | in_progress | Governance |
