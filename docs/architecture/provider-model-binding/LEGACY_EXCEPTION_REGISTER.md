...
| LC-07 | `server/automation/block-executors.ts:102` | Automation | Bare string `"catalog_entries"` in `ALLOWED_TABLES` — used for SQL identifier validation in a generic DB-block executor. | Phase 19 verified: this is a SQL target whitelist for a power-user automation block, not a code path that READS catalog_entries. Behavior matches the surrounding allowed-table list (workspaces, agents, documents, etc.). | Low (whitelist entry, not an active read/write). | Either: (a) accept as catalog-touch-via-allowlist and document; (b) remove if catalog_entries access via raw SQL is undesirable (DELETE/UPDATE blocks could hit it). | Phase 25 (catalog access boundary enforcement) | TBD | open | Reviewer |
