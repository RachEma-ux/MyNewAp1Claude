...
|---|---|---|---|---|---|---|---|---|---|---|
| LR-01 | `server/agent-studio/adapters/openllm-runtime-adapter.ts:312-321` | Agent Studio | Reads `process.env[pc.apiKeyEnvVar]` for runtime provider auth. | Active runtime path used by Expert chat (PR #100 path). Cannot remove until Model Access is live. | High — exact path PR #100 incident exploited. | `withProviderCredential()` via `openRouter.modelAccess.execute`. | Phase 17 (Expert chat through Model Access) | TBD | open | Governance |
