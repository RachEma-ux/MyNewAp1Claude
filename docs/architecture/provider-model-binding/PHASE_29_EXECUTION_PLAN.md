...
- [ ] **29.6d — `chat-binding.test.ts` cleanup (deferred).** The "fall through to legacy path" tests at lines 184–211 reference behavior that no longer exists post-29.6a. They sit on a pre-existing-red CI shard; the cleanup is non-blocking and can land as a §29.7 follow-up alongside the boundary-lint sweep.
