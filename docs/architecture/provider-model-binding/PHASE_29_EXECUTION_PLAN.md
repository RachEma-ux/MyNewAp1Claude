...
### 29.6 — LR-08 `/api/chat/stream` + `executeInvokeAgent` — **CLOSED**

Shipped:

- [x] **29.6a — `chat/stream.ts` migrated.** Direct-imports `stream()` from `../openrouter/model-access` (gateway-call collapses streams to a single result; SSE requires real streaming). Binding resolves via D-PR-4 two-step lookup: (I) `listActiveForProvider({workspaceId, providerCatalogEntryId: providerId})` for the legacy `providerId`; (II) `resolveWorkspaceDefaultBinding({workspaceId, role:"chat"})` fallback. Refuses with `binding_required` when both miss. Intent `"chat"` (semantically accurate). `providerRouter.resolvePlan` selection preserved for `useUnifiedRouting`; only the execution step changed. SSE shape preserved (token + complete events); RAG-context injection unchanged; routing audit reasons echoed in the complete event. Cost-tracking simplified — token counts tracked, per-model cost calculation moved to a Phase-30 follow-up alongside 29.1c admin UI (the previous `provider.getCostPerToken()` registry hook is gone with the registry usage).
- [x] **29.6b — `executeInvokeAgent` migrated (Path B).** Per D-PR-5 ADR (refuse) — the `agents` table is pre-Agent-Studio (CLAUDE.md: out-of-retrofit-scope). Returns a soft-error result with `error: "legacy_agents_table_unsupported"`; admins migrate workflows to AS agents. Path B preferred over Path A (backfill is non-trivial for out-of-scope code) and Path C (dual-table re-introduces the registry path while we're closing it). Pre-condition check documented inline: admins enumerate active legacy-agent workflows in `workflow_executions` before production rollout; if non-zero, surface for sign-off.
- [x] **29.6c — Boundary fences.** `tests/pmb/boundary.test.ts` adds 5 new tripwire tests under "Phase 29.6 invariant — LR-08 closure": (1) `chat/stream.ts` does not import `getProviderRegistry`; (2) chat-stream direct-imports `stream` from model-access with `intent: "chat"`; (3) chat-stream uses both `resolveWorkspaceDefaultBinding` and `listActiveForProvider`; (4) `executeInvokeAgent` does not import `getProviderRegistry` (Path B); (5) `executeInvokeAgent` returns the `legacy_agents_table_unsupported` refusal shape. LR-08 has no boundary-lint allowlist entry to remove (per the original Phase 29 plan note — the registry consumption was one layer below the lint's surface).
- [x] **29.6d — `chat-binding.test.ts` partial cleanup.** Closed in §29.7 PR. Two of the previously-failing tests (FORBIDDEN_BINDING_KEYS mock + rac-orchestrator DB-dependency) fixed; 5 of 7 tests now pass (was 0/7). The remaining 2 failures are tool-loop assertions about post-error chat-message-row writes that aren't related to the receipt-policy/binding migration; they need the test logic updated to match the post-29.6 chat shape and are documented as a §B-followup test-infra cleanup item, NOT blocking. Note: this test file is NOT in the CI test set (`run-tests.yml` only runs `tests/contracts/`, `tests/governance/`, `tests/integration/*`, `tests/ui/`; `server/**` tests are local-development only), so neither the original failures nor the remaining 2 affect CI-fingerprint.
- [x] **Acceptance:** `pnpm run check` clean; boundary lint clean (only pre-existing untracked-file violation remains); 27/27 tests green in boundary.test.ts (5 new tripwires) + 24/24 in `server/chat/` (existing chat tests unchanged by the refactor).
- [ ] **Live-smoke pending:** exercise legacy chat UI at `client/src/pages/Chat.tsx` (both unified-routing ON and OFF paths) + an automation workflow that calls `executeInvokeAgent` (confirm Path B refusal surfaces cleanly in the workflow execution log).
- [x] **Net diff:** ~5 files changed, ~250 insertions, ~110 deletions. The chat-stream rewrite swaps roughly equal LOC; the executeInvokeAgent change is a net deletion (provider-chain logic gone).

### 29.7 — Boundary-lint allowlist purge — **CLOSED**

Shipped:

- [x] **Final allowlist sweep audit.** Per-sub-phase removals already done: LR-01 in 29.0a; LR-02 + LR-03 in 29.4b; LR-04 in 29.5. LR-08 never had an allowlist entry (registry consumption was one layer below the lint's surface). Final state: **`PMB-D1-EXEMPT` for `scripts/provider-connections/seed-from-env.ts` is the ONLY remaining allowlist entry** (the one legitimate boot-time env reader by design).
- [x] **Boundary-lint comments updated.** `scripts/check-provider-key-env-boundary.ts` carries closure notes for LR-01 / LR-02 + LR-03 / LR-04 / LR-08, plus a Phase 29 summary block stating the final state. Each note documents the migration shape so a future regression can be diagnosed from the lint output alone.
