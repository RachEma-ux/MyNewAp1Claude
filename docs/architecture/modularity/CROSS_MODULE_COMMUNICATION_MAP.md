...
| Any module      | Data Analysis   | gateway      | receipt for `dataAnalysis.dataAcquisition.runProcessing / runOutputPipeline / document.runParser / document.runOutputPipeline / exportCanonicalRecord` | Read Data Acquisition state and submit acquisition / classification / routing / processing / output runs through `dataAnalysis.dataAcquisition.*` gateway actions. Document Intelligence is exposed under `*.document.*`. No module may write `data_acquisition_*` tables directly. |
| Data Analysis   | All modules     | event        | none                 | `dataAnalysis.dataAcquisition.sourceRegistered`, `*.acquisition*`, `*.itemDiscovered/Classified/Routed`, `*.processing*`, `*.canonicalRecordCreated`, `*.qualityValidated`, `*.outputPipeline*`, `*.workerUnavailable/Recovered`, `*.document.parserSelected/parserFallbackTriggered/reconstructed/validated` — fan-out to subscribers (HQ, GraphRAG, KGRA, audit). |
| Data Analysis   | Data Acquisition worker (`:8485`) | port (external) | none       | Data Acquisition worker is a Data Analysis runtime endpoint (sibling of the GraphRAG worker), not its own module. Capabilities: `classify / route / parse / ocr / reconstruct / validate / output`. Missing worker → Data Acquisition processing runs recorded as `failed`/`degraded`. |
| Data Acquisition (subdomain) | GraphRAG (subdomain) | event | none      | `outputPipelineCompleted` with `outputType=graphrag` carries a canonical record reference; GraphRAG ingests via subscriber. No direct cross-subdomain function calls. |
