/**
 * Culture & Values Capsule — Structural invariants.
 */
...
const CV_DIR = join(REPO_ROOT, "client", "src", "modules", "culture-values");
const APP_PATH = join(REPO_ROOT, "client", "src", "App.tsx");

describe("CV capsule files exist", () => {
  it.each([
    "client.ts",
    "manifest.ts",
    "mod.tsx",
    ...
    expect(existsSync(join(CV_DIR, rel))).toBe(true);
  });
  ...
  it.each([
    ...
    expect(existsSync(join(CV_DIR, "pages", rel))).toBe(true);
  });
});

describe("CV migration state", () => {
  it.each([
    "cultureValues",
...
describe("CV manifest", () => {
  // readManifestFor(repoRoot, RtlmKey camelCase, folder kebab-case)
  const snap = readManifestFor(REPO_ROOT, "cultureValues", "culture-values");

  it("declares baseRoute /cv", () => {
    expect(snap.baseRoute).toBe("/cv");
  });
  ...
  it("routeInventory contains the 4 canonical routes", () => {
...
describe("App.tsx ownership", () => {
  ...
  const folder = RTLM_FOLDER_MAP.cultureValues;

  it("does not lazy-import any private CV capsule page", () => {
    expect(appSrc).not.toMatch(
      new RegExp(`import\\([^)]*?modules/${folder}/pages/`),
    );
  });

  it("does not lazy-import legacy @/pages/culture-values/ pages", () => {
    expect(appSrc).not.toMatch(/import\([^)]*?@\/pages\/culture-values\//);
  });

  it("does not mount any canonical /cv/* route owned by CV", () => {
    const snap = readManifestFor(REPO_ROOT, "cultureValues", "culture-values");
...
describe("Boundary discipline", () => {
  const findings = collectAllBoundaryFindings(CV_DIR);
