...
/**
 * Catalog Entries - Authoring space for provider/model configurations
 * Each entry represents a provider+model combo being configured before publishing
 */
export const catalogEntries = pgTable("catalog_entries", {
  ...
  validationStatus: varchar("validationStatus", { length: 50 }), // pending, passed, failed
...
/**
 * Catalog Entry Versions - Immutable snapshots of catalog entry configurations
 * Created each time a catalog entry is modified (for audit trail)
 */
export const catalogEntryVersions = pgTable("catalog_entry_versions", {
  id: serial("id").primaryKey(),
  catalogEntryId: integer("catalogEntryId").notNull(),

  // Version number (auto-incremented per entry)
  version: integer("version").notNull(),
...
/**
 * Publish Bundles - Immutable, versioned snapshots published to the registry
 * Contains a complete frozen configuration with hash for consumption
 */
export const publishBundles = pgTable("publish_bundles", {
  id: serial("id").primaryKey(),
  catalogEntryId: integer("catalogEntryId").notNull(),

  // Version label (semver or sequential)
  ...
  // Publishing metadata
  status: varchar("status", { length: 50 }).default("active").notNull(), // active, superseded, recalled
  publishedBy: integer("publishedBy").notNull(),
  publishedAt: timestamp("publishedAt").defaultNow().notNull(),

  // Policy gate results at publish time
...
/**
 * Catalog Audit Events - Immutable audit trail for all catalog management actions
 */
export const catalogAuditEvents = pgTable("catalog_audit_events", {
  id: serial("id").primaryKey(),

  // Event classification
  eventType: varchar("eventType", { length: 100 }).notNull(), // catalog.entry.created, catalog.entry.validated, catalog.bundle.published, etc.
...
/**
 * Import Sessions — Tracks each import wizard session.
 * UUID PK generated by the app (crypto.randomUUID).
 */
export const importSessions = pgTable("import_sessions", {
  id: text("id").primaryKey(), // UUID generated by app
  userId: integer("userId").notNull(),
  method: varchar("method", { length: 50 }).notNull(), // api_discovery, file_upload, registry_sync, openapi_spec
  sourceRef: text("sourceRef").notNull(), // URL, file path, etc.
  status: varchar("status", { length: 50 }).default("pending").notNull(),
  version: integer("version").default(1).notNull(), // optimistic lock
  summary: json("summary"), // PreviewSummary JSON
  error: text("error"),
  expiresAt: timestamp("expiresAt").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
...
/**
 * Import Preview Rows — Staged entries awaiting user selection.
 * Cascade-deleted when the parent session is removed.
 */
export const importPreviewRows = pgTable("import_preview_rows", {
  id: serial("id").primaryKey(),
  sessionId: text("sessionId").notNull(), // FK to importSessions.id (cascade in migration SQL)
  tempId: text("tempId").notNull(), // client-generated unique id
  type: varchar("type", { length: 50 }).notNull(), // provider, model, llm, etc.
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description"),
  source: varchar("source", { length: 100 }).notNull(), // e.g., openai_api
...
/**
 * Import Audit Logs — Immutable record of each completed import operation.
 */
