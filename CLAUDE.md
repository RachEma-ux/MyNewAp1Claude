...
- OCR-PDF parser — listed in D-UI-5's original deferral set; §D4 closed audio/video/image-OCR but PDF-with-images stays open behind a future `D-PARSE-OCRPDF-N` ADR. (DOCX closed at R5 — `D-PARSE-DOCX-1..4` per `docs/architecture/agent-studio-docx-parser.md`.)
