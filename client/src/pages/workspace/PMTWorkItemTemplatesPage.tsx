/**
 * PMT Work Item Templates — Reusable task blueprints
 */
...
export function PMTWorkItemTemplatesPage() {
  ...
  const updateMut = trpc.modules.pmt.templates.workItemTemplates.update.useMutation({
    onSuccess: () => { utils.modules.pmt.templates.workItemTemplates.list.invalidate(); setDialogOpen(false); toast.success("Template updated"); },
    onError: (e) => toast.error(e.message),
  });

  const deleteMut = trpc.modules.pmt.templates.workItemTemplates.delete.useMutation({
    onSuccess: () => { utils.modules.pmt.templates.workItemTemplates.list.invalidate(); toast.success("Template deleted"); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const openCreate = () => {
    setEditingId(null);
    setForm({ name: "", description: "", templateData: "{}", projectId: String(projects?.[0]?.id || "") });
    setDialogOpen(true);
  };
            ...
            <Card key={t.id}>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center justify-between">
                  ...
                  <Badge variant="outline" className="text-[10px]">Item Template</Badge>
                </CardTitle>
              </CardHeader>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder='{"title": "Default Task", "status": "todo", "priority": "medium"}'
              />
            </div>
          </div>
          <DialogFooter>
