/**
 * PRM AI Catalog — Import AI Types into PRM (cloned from SandboxWF AI Catalog).
 * Uses prm.catalogImports tRPC router (PRMDB-backed).
 */
...
export default function PRMAICatalogPage() {
  const utils = trpc.useUtils();
  const [showPicker, setShowPicker] = useState(false);
  const [pickerSearch, setPickerSearch] = useState("");
  const [pickerTab, setPickerTab] = useState("all");

  const importsQuery = trpc.prm.catalogImports.list.useQuery();
  const catalogImports = importsQuery.data ?? [];

  const importMutation = trpc.prm.catalogImports.import.useMutation({
    onSuccess: (data: any) => {
      utils.prm.catalogImports.list.invalidate();
      toast.success(`Imported "${data?.name || "entry"}" into PRM`);
    },
    onError: (err) => {
  ...
  const removeMutation = trpc.prm.catalogImports.remove.useMutation({
    onSuccess: () => {
      utils.prm.catalogImports.list.invalidate();
    },
  });
