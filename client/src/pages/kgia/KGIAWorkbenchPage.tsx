...
import { Loader2, Send, Eye, FileSearch, Clock, AlertTriangle } from "lucide-react";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if ("envelope" in result) {
        ...
        if (result.envelope) {
          onQueryPlanReady(null); // Plans come from run detail
        }
        ...
        <ScrollArea className="flex-1">
          <div className="p-1.5 space-y-0.5">
            {(runsQuery.data ?? []).map((run) => (
              ...
              >
                <div className="truncate">{run.question.slice(0, 50)}</div>
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  {run.confidence !== null && (
                    <span className="text-[9px]">{(run.confidence * 100).toFixed(0)}%</span>
                  )}
                </div>
              </button>
            ))}
          ...
          <div className="flex gap-2">
            ...
            {sourcesQuery.data && sourcesQuery.data.length > 0 && (
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                <span>{sourcesQuery.data.length} source{sourcesQuery.data.length !== 1 ? "s" : ""} available</span>
              </div>
            )}
          </div>
        </div>
