/**
 * PRM Sidebar — Simple IBM Shell sidebar (cloned from PSSidebar)
 */
...
const NAV_ITEMS: { key: PRMView; label: string; icon: React.ElementType }[] = [
  ...
  { key: "new", label: "New Case", icon: PlusCircle },
...
interface Props {
  active: PRMView;
  onNavigate: (key: PRMView) => void;
  collapsed: boolean;
  onToggle: () => void;
}

export default function PRMSidebar({ active, onNavigate, collapsed, onToggle }: Props) {
  return (
    <div
      className={cn(
