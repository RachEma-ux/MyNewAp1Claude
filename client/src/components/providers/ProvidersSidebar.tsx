/**
 * Providers Sidebar — Simple IBM Shell sidebar
 * Cloned from AITypesSidebar pattern. w-12 collapsed / w-56 expanded.
 */
...
export type ProviderView = "dashboard" | "control-panel" | "wizard" | "list";

const NAV_ITEMS: { key: ProviderView; label: string; icon: React.ElementType }[] = [
  { key: "dashboard", label: "Dashboard", icon: BarChart3 },
  { key: "control-panel", label: "Control Panel", icon: Settings },
  { key: "wizard", label: "Wizard", icon: Wand2 },
  { key: "list", label: "Providers List", icon: List },
];

interface Props {
  active: ProviderView;
  onNavigate: (key: ProviderView) => void;
  collapsed: boolean;
  onToggle: () => void;
}

export default function ProvidersSidebar({ active, onNavigate, collapsed, onToggle }: Props) {
  return (
    <div
      className={cn(
      ...
      <div className={cn("flex items-center border-b", collapsed ? "justify-center py-1.5" : "justify-between px-2 py-1.5")}>
        {!collapsed && (
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-xs font-semibold text-muted-foreground truncate">Providers</span>
          </div>
        )}
