/**
 * Bots Sidebar — Simple IBM Shell sidebar
 */
...
export type BotView = "dashboard" | "control-panel" | "wizard" | "list" | "analytics";

const NAV_ITEMS: { key: BotView; label: string; icon: React.ElementType }[] = [
  { key: "dashboard", label: "Dashboard", icon: BarChart3 },
  { key: "control-panel", label: "Control Panel", icon: Settings },
  { key: "wizard", label: "Wizard", icon: Wand2 },
  { key: "list", label: "Bots List", icon: List },
  { key: "analytics", label: "Analytics", icon: LineChart },
];

interface Props {
  active: BotView;
  onNavigate: (key: BotView) => void;
  collapsed: boolean;
  onToggle: () => void;
}

export default function BotsSidebar({ active, onNavigate, collapsed, onToggle }: Props) {
  return (
    <div
      className={cn(
      ...
      <div className={cn("flex items-center border-b", collapsed ? "justify-center py-1.5" : "justify-between px-2 py-1.5")}>
        {!collapsed && (
          <div className="flex items-center gap-2 min-w-0">
            <MessageSquare className="h-3.5 w-3.5 shrink-0 text-pink-500" />
            <span className="text-xs font-semibold text-muted-foreground truncate">Bots</span>
          </div>
        )}
