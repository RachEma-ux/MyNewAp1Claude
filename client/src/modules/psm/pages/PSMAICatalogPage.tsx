/**
 * PSM AI Catalog — Import AI Types into PSM (cloned from PRM AI Catalog).
 * Uses psm.catalogImports tRPC router (PSMDB-backed).
 */
...
export default function PSMAICatalogPage() {
  const utils = trpc.useUtils();
  const [showPicker, setShowPicker] = useState(false);
  const [pickerSearch, setPickerSearch] = useState("");
  const [pickerTab, setPickerTab] = useState("all");

  const importsQuery = trpc.psm.catalogImports.list.useQuery();
  const catalogImports = importsQuery.data ?? [];

  const importMutation = trpc.psm.catalogImports.import.useMutation({
    onSuccess: (data: any) => {
      utils.psm.catalogImports.list.invalidate();
      toast.success(`Imported "${data?.name || "entry"}" into PSM`);
    },
    onError: (err) => {
  ...
  const removeMutation = trpc.psm.catalogImports.remove.useMutation({
    onSuccess: () => {
      utils.psm.catalogImports.list.invalidate();
    },
  });
