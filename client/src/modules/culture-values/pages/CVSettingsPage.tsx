/**
 * CV Settings — Module feature flags and configuration
 */
...
export function CVSettingsPage({ workspaceId }: { workspaceId: number }) {
  const settings = trpc.cultureValues.settings.get.useQuery({ workspaceId });
  ...
  return (
    <div className="space-y-4 p-4">
      <h1 className="text-2xl font-bold">Culture Values Settings</h1>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="text-sm text-muted-foreground">
            Module: <span className="font-medium text-foreground">{settings.data?.module?.toUpperCase() ?? "CV"}</span>
          </div>
        </CardContent>
      </Card>
