/**
 * MaestroChatWindow — Floating multi-AI chat window for WF Sandbox
 *
 * Intercom-style FAB at bottom-right; expands into a chat panel.
 * Users pick participants from imported WF catalog entries,
...
interface MaestroChatWindowProps {
...
export function MaestroChatWindow({ catalogImports }: MaestroChatWindowProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      toast.error(`Maestro error: ${err.message}`);
    } finally {
    ...
    <div className="fixed bottom-4 right-4 z-50 w-[380px] h-[520px] flex flex-col rounded-xl border bg-card shadow-2xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b bg-card shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Maestro Chat</span>
          {selectedIds.size > 0 && (
        ...
        <div className="flex flex-wrap gap-1">
          {activeImports.length === 0 ? (
            <span className="text-[10px] text-muted-foreground italic">
              No AI types imported — import from Catalog first
            </span>
          ) : (
