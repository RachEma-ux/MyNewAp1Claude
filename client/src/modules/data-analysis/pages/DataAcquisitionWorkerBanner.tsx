 ...
 * worker (default `http://localhost:8485`) is unavailable. The UI
 * remains usable; processing-bound operations record failed/degraded
 * runs rather than crash.
 */
...
export function DataAcquisitionWorkerBanner() {
  const status =
  ...
  const data = status.data as { healthy: boolean; url: string; message: string } | undefined;
  if (!data) return null;
